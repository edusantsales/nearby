...
class HomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        homeView.bodyView.configureTableViewDelegate(self, dataSource: self)
        defineInitialLocation()

        homeViewModel.getInitialData { [weak self] categories in
            guard let self = self else { return }
            self.homeView.headerView.updateFilterButtons(with: categories) {
                selectedCategory in
        ...
        homeViewModel.didUpdatePlaces = { [weak self] in
            DispatchQueue.main.async {
                ...
                self?.homeView.bodyView.reloadTableViewData()
                self?.addAnnotationsToMap()
            }
        }
    }
...
extension HomeViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(
        ...
        self.flowDelegate?.goToPlaceDetails(place: places[indexPath.row])
    }
}
